...
for lt in (2000,3000,3500,4000,4500,5000,6000):
    ...
    c.Divide(2,2)
    pad1 = c.cd(1)
    hreg150.SetLineColor(kBlack)
    hinf150.SetLineColor(kRed+1)
    hinf150.SetLineStyle(4)
    for h in (hreg150,hinf150):
        h.SetLineWidth(2)
    ...
    # # pad1.SetLogy()
    c.cd(2)
    hsig150.SetTitle("(#Lambda_{T}=%i) - (#Lambda_{T}=#infty)"%lt)
    hsig150.GetYaxis().SetTitle("(#Lambda_{T}=%i) - (#Lambda_{T}=#infty)"%lt)
    # print xssig
    ...
    pad3 = c.cd(3)
    # print hinf.Integral()
    hbkgint = hinf150.Clone()
    hbkgint.SetName("hbkgint")
    hbkgint.SetTitle("")
    endBin = hinf150.GetNbinsX()+1
    for i in range( 1, endBin):
        x = hinf150.Integral(i,endBin)
        hbkgint.SetBinContent(i,x)
    pad3.SetLogy()
    hbkgint.Draw()

    c.cd(4)
    hsigint = hsig150.Clone()
    hsigint.SetName("hsigint")
    hsigint.SetTitle("")
    endBin = hsig150.GetNbinsX()+1
    for i in range( 1, endBin):
        x = hsig150.Integral(i,endBin)
        hsigint.SetBinContent(i,x)
    hsigint.Draw()

    c.SaveAs("massplot_sig_pTHat150-500_LambdaT%i.png"%(lt))
    # freg.Close()
    # finf.Close()